...
contract('FmhToken', function(accounts){
	...
	it('initializes the contract with the correct values', function(){
		return FmhToken.deployed().then(function(instance) {
			...
			assert.equal(name, 'FMH Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'FMH', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard, 'FMH Token v1.0', 'has the correct standard');
		});
	});

	it('allocates the initial supply upon deployement', function(){
		return FmhToken.deployed().then(function(instance){
			tokenInstance = instance;
	...
	it('handles delegated token tranfer', function() {
		return FmhToken.deployed().then(function(instance) {
			tokenInstance = instance;
